...
ospf6_as_external_lsa_originate (struct ospf6_route *route)
{
  ...
  if (IS_OSPF6_DEBUG_ASBR || IS_OSPF6_DEBUG_ORIGINATE (AS_EXTERNAL))
    {
      ospf6_prefix2str (ospf6, &route->prefix, buf, sizeof (buf));
...
ospf6_asbr_lsa_add (struct ospf6_lsa *lsa)
{
  ...
  if (ospf6_af_validate_prefix (ospf6, &prefix,
                                external->prefix.prefix_length, true))
    {
      ...
      ospf6_addr2str6 (&prefix, buf, sizeof (buf));
      zlog_warn ("%s: ignoring prefix %s/%u in lsa %s: "
		 "address family incompatibility", __func__,
		 buf, external->prefix.prefix_length, lsa->name);
  ...
  if (asbr_entry == NULL ||
      ! CHECK_FLAG (asbr_entry->path.router_bits, OSPF6_ROUTER_BIT_E))
    {
      if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
        {
	  ospf6_id2str (ospf6_adv_router_in_prefix (&asbr_id),
			buf, sizeof (buf));
  ...
  if (ospf6_af_validate_prefix (ospf6, &prefix.u.prefix6,
                                prefix.prefixlen, true))
    {
      ...
      zlog_warn ("%s: ignoring prefix %s in lsa %s: "
		 "address family incompatibility", __func__, buf, lsa->name);
  ...
  if (route == NULL)
    {
      if (IS_OSPF6_DEBUG_EXAMIN (AS_EXTERNAL))
        {
          ospf6_prefix2str (ospf6, &prefix, buf, sizeof (buf));
...
static void
ospf6_asbr_routemap_set (struct ospf6 *o, int type, const char *mapname)
{
  if (o->rmap[type].name)
    XFREE (MTYPE_OSPF6_OTHER, o->rmap[type].name);
  o->rmap[type].name = XSTRDUP (MTYPE_OSPF6_OTHER, mapname);
  o->rmap[type].map = route_map_lookup_by_name (mapname);
}

static void
ospf6_asbr_routemap_unset (struct ospf6 *o, int type)
{
  if (o->rmap[type].name)
    XFREE (MTYPE_OSPF6_OTHER, o->rmap[type].name);
  o->rmap[type].name = NULL;
  o->rmap[type].map = NULL;
}
...
void
ospf6_asbr_external_table_finish (struct ospf6_route_table *external_table)
{
  struct ospf6_route *route;

  for (route = ospf6_route_head (external_table); route;
       route = ospf6_route_next (route))
    {
      struct ospf6_external_info *info;

      info = route->route_option;
      ospf6_route_remove (route, external_table);
      XFREE (MTYPE_OSPF6_EXTERNAL_INFO, info);
    }
}

void
ospf6_asbr_delete (struct ospf6 *o)
{
  int type;

  for (type = 0; type < ZEBRA_ROUTE_MAX; type++)
    ospf6_asbr_routemap_unset (o, type);
}
...
void
ospf6_asbr_redistribute_enable (struct ospf6 *o)
{
  int type;

  assert (!CHECK_FLAG (o->flag, OSPF6_DISABLED));
...
void
ospf6_asbr_redistribute_disable (struct ospf6 *o)
{
  int type;
...
static void
ospf6_asbr_redistribute_set (struct ospf6 *o, int type)
{
  o->redist[type] = 1;
...
static void
ospf6_asbr_redistribute_unset (struct ospf6 *o, int type)
{
  struct ospf6_route *route;
  struct ospf6_external_info *info;
...
static u_int32_t
ospf6_asbr_lsid (struct ospf6 *o, struct prefix *prefix)
{
  struct ospf6_lsa *lsa;
  u_int32_t router_id, lsid;
  u_int16_t type;

  router_id = o->router_id;
  type = htons (OSPF6_LSTYPE_AS_EXTERNAL);

  for (lsa = ospf6_lsdb_type_router_head (type, router_id, o->lsdb);
       lsa != NULL;
	...
	(struct ospf6_as_external_lsa *) OSPF6_LSA_HEADER_END (lsa->header);
      ...
      lsa_prefix.prefixlen = as_external_lsa->prefix.prefix_length;
      memcpy (&lsa_prefix.u.prefix6,
	      OSPF6_PREFIX_BODY (&as_external_lsa->prefix),
	      OSPF6_PREFIX_SPACE (as_external_lsa->prefix.prefix_length));

      if (prefix_cmp (prefix, &lsa_prefix) == 0)
	break;

      lsid = ntohl (lsa->header->id);
      if (o->external_id <= lsid)
	o->external_id = lsid + 1;
    }
  ...
  if (lsa != NULL)
  ...
  else
    {
      lsid = o->external_id;
      o->external_id++;
      assert (ospf6_lsdb_lookup (type, htonl (lsid),
				 router_id, o->lsdb) == NULL);
    }
                             ...
                             u_int32_t metric)
{
  ...
  if (!ospf6 || CHECK_FLAG (ospf6->flag, OSPF6_DISABLED))
    return;
  ...
  if (ospf6_af_validate_prefix (ospf6, &prefix->u.prefix6,
                                prefix->prefixlen, true))
    {
      ...
      prefix2str (prefix, buf, sizeof(buf));
      zlog_warn ("%s: ignoring %s prefix %s: "
		 "address family incompatibility", __func__,
		 zebra_route_string (type), buf);

      /* ignore this prefix */
      return;
    }

  /* Don't redistribute OSPF interface prefixes as external */
  oi = ospf6_interface_lookup_by_ifindex (ifindex);
  if (oi != NULL && ospf6_interface_prefix_is_connected (oi, prefix))
    return;

  if (IS_OSPF6_DEBUG_ASBR)
    {
      ospf6_prefix2str (ospf6, prefix, pbuf, sizeof (pbuf));
  ...
  if (match)
    {
      ...
      if (IS_OSPF6_DEBUG_ASBR)
        {
	  ospf6_id2str (prefix_id.u.prefix4.s_addr, ibuf, sizeof (ibuf));
  ...
  info->id = ospf6_asbr_lsid (ospf6, prefix);
...
void
ospf6_asbr_redistribute_remove_connected (struct ospf6_interface *oi)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, n, c))
      ...
      if (family == AF_INET)
	{
	  ...
	  err = ospf6_af_prefix_convert4to6 ((struct prefix_ipv6 *)&tmp,
      ...
      else
	{
      ...
      ospf6_asbr_redistribute_remove (ZEBRA_ROUTE_CONNECT, ifp->ifindex, p);
    }
}
       ...
       QUAGGA_REDIST_HELP_STR_OSPF6D)
{
  ...
  ospf6_asbr_redistribute_unset (o, type);
  ospf6_asbr_routemap_unset (o, type);
  ospf6_asbr_redistribute_set (o, type);
  return CMD_SUCCESS;
}
       ...
       "Route map name\n")
{
  ...
  ospf6_asbr_redistribute_unset (o, type);
  ospf6_asbr_routemap_set (o, type, argv[1]);
  ospf6_asbr_redistribute_set (o, type);
  return CMD_SUCCESS;
}
  ...
  if (ospf6_af_is_ipv4 (ospf6))
    {
      ...
      r = inet_aton (forwarding, &addr4);
      if (r)
	{
	  ospf6_af_address_convert4to6 (&info->forwarding, &addr4);
	  r = 1;
	}
    }
  else
    {
      r = inet_pton (AF_INET6, forwarding, &info->forwarding);
    }
...
DEFUN (ospf6_routemap_set_forwarding,
       ...
       "set forwarding-address (X:X::X:X|A.B.C.D)",
       "Set value\n"
       "Forwarding Address\n"
       "IPv6 Address\n"
       "IPv4 Address\n")
...
DEFUN (ospf6_routemap_no_set_forwarding,
       ...
       "no set forwarding-address (X:X::X:X|A.B.C.D)",
       NO_STR
       "Set value\n"
       "Forwarding Address\n"
       "IPv6 Address\n"
...
ospf6_as_external_lsa_show (struct vty *vty, struct ospf6_lsa *lsa)
{
  ...
  vty_out (vty, "     Prefix: %s/%d%s", buf,
           ospf6_af_prefixlen6 (ospf6, external->prefix.prefix_length), VNL);
  ...
  if (CHECK_FLAG (external->bits_metric, OSPF6_ASBR_BIT_F))
    {
      ...
      ospf6_addr2str (ospf6, forwarding, buf, sizeof (buf));
...
ospf6_asbr_external_route_show (struct vty *vty, struct ospf6_route *route)
{
  ...
  ospf6_prefix2str (ospf6, &route->prefix, prefix, sizeof (prefix));
  tmp_id = ntohl (info->id);
  ospf6_id2str (tmp_id, id, sizeof (id));
  if (! IN6_IS_ADDR_UNSPECIFIED (&info->forwarding))
    ospf6_addr2str (ospf6, &info->forwarding, forwarding, sizeof (forwarding));
  else
