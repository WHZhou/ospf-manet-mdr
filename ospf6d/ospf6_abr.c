...
static int
ospf6_abr_inter_prefix_cmp (struct prefix *prefix, struct ospf6_lsa *lsa)
{
  struct ospf6_inter_prefix_lsa *inter_prefix_lsa;
  ...
  lsa_prefix.prefixlen = inter_prefix_lsa->prefix.prefix_length;
  memcpy (&lsa_prefix.u.prefix6,
	  OSPF6_PREFIX_BODY (&inter_prefix_lsa->prefix),
	  OSPF6_PREFIX_SPACE (inter_prefix_lsa->prefix.prefix_length));

  return prefix_cmp (prefix, &lsa_prefix);
}

static int
ospf6_abr_inter_router_cmp (struct prefix *prefix, struct ospf6_lsa *lsa)
{
  u_int32_t router_id;
  struct ospf6_inter_router_lsa *inter_router_lsa;

  router_id = ospf6_adv_router_in_prefix (prefix);

  inter_router_lsa =
    (struct ospf6_inter_router_lsa *) OSPF6_LSA_HEADER_END (lsa->header);

  return ntohl (router_id) - ntohl (inter_router_lsa->router_id);
}

static int
ospf6_abr_inter_lsa_cmp (struct prefix *prefix, struct ospf6_lsa *lsa)
{
  u_int16_t type;
  int cmp;
  ...
  switch (type)
    ...
    case OSPF6_LSTYPE_INTER_PREFIX:
      cmp = ospf6_abr_inter_prefix_cmp (prefix, lsa);
      break;

    case OSPF6_LSTYPE_INTER_ROUTER:
      cmp = ospf6_abr_inter_router_cmp (prefix, lsa);
      break;
...
static u_int32_t
ospf6_abr_lsid (struct ospf6_area *area,
		struct prefix *prefix, u_int16_t type)
{
  struct ospf6_lsa *lsa;
  u_int32_t router_id, lsid;

  router_id = area->ospf6->router_id;

  for (lsa = ospf6_lsdb_type_router_head (type, router_id, area->lsdb);
       lsa != NULL;
       lsa = ospf6_lsdb_type_router_next (type, router_id, lsa))
    {
      if (ospf6_abr_inter_lsa_cmp (prefix, lsa) == 0)
	break;

      lsid = ntohl (lsa->header->id);
      if (area->inter_area_id <= lsid)
	area->inter_area_id = lsid + 1;
    }
  ...
  if (lsa != NULL)
    ...
    {
      lsid = htonl (area->inter_area_id);
      area->inter_area_id++;
      assert (ospf6_lsdb_lookup (type, lsid, router_id, area->lsdb) == NULL);
    }
                                     ...
                                     struct ospf6_area *area)
{
  ...
  if (route->type == OSPF6_DEST_TYPE_ROUTER)
    {
      if (IS_OSPF6_DEBUG_ABR || IS_OSPF6_DEBUG_ORIGINATE (INTER_ROUTER))
        {
          is_debug++;
	  ospf6_id2str (ospf6_adv_router_in_prefix (&route->prefix),
			buf, sizeof (buf));
          zlog_debug ("Originating summary in area %s for ABR %s",
		      area->name, buf);
        }
  ...
  else
    {
      if (IS_OSPF6_DEBUG_ABR || IS_OSPF6_DEBUG_ORIGINATE (INTER_PREFIX))
        {
          is_debug++;
          ospf6_prefix2str (area->ospf6, &route->prefix,
			    buf, sizeof (buf));
  ...
  if (!CHECK_FLAG (route->flag, OSPF6_ROUTE_BEST))
    {
      /*
       * This is not a new best route being added or a previous best
       * route that was removed
       */
      if (is_debug)
        zlog_debug ("The route is not best, ignore");
      return;
    }
  ...
  if (CHECK_FLAG (route->flag, OSPF6_ROUTE_WAS_REMOVED))
    {
      struct ospf6_route *best;

      /* check if another route to the prefix exists */
      best = ospf6_route_lookup (&route->prefix, route->table);
      if (best == NULL)
	{
	  /* no route found to the prefix */
	  if (is_debug)
	    zlog_debug ("The route was removed, purge previous LSA");
	  if (summary)
	...
	{
	  /* an alternate route exists to the prefix */

	  /* TODO: Could check if summary actually needs to be
	   * updated.  This doesn't matter too much since redundant
	   * LSAs should be suppressed
	   */

	  /* treat best as being added below */
	  route = best;
	  if (is_debug)
	    zlog_debug ("The route was removed, updating "
			"summary with current best route");
	}
    }
  ...
  if (route->type == OSPF6_DEST_TYPE_NETWORK &&
      route->path.type == OSPF6_PATH_TYPE_INTRA)
    {
      ...
      if (range && ! CHECK_FLAG (range->flag, OSPF6_ROUTE_WAS_REMOVED) &&
          (route->path.area_id != BACKBONE_AREA_ID ||
           ! IS_AREA_TRANSIT (area)))
        {
          if (is_debug)
            {
              ospf6_prefix2str (area->ospf6, &range->prefix,
				buf, sizeof (buf));
  ...
  if (summary == NULL)
    {
      ...
      summary->path.origin.id =
        ospf6_abr_lsid (area, &summary->prefix, summary->path.origin.type);
  ...
  if (route->type == OSPF6_DEST_TYPE_ROUTER)
    {
      ...
      router_lsa->router_id = ospf6_adv_router_in_prefix (&route->prefix);
  ...
  for (ro = ospf6_route_match_head (&range->prefix, ospf6->route_table);
       ro; ro = ospf6_route_match_next (&range->prefix, ro))
    {
      if (ro->path.area_id == range->path.area_id &&
          ! CHECK_FLAG (ro->flag, OSPF6_ROUTE_WAS_REMOVED))
...
ospf6_abr_examin_summary (struct ospf6_lsa *lsa, struct ospf6_area *oa)
{
  ...
  if (lsa->header->type == htons (OSPF6_LSTYPE_INTER_PREFIX))
  ...
  else if (lsa->header->type == htons (OSPF6_LSTYPE_INTER_ROUTER))
    {
      ...
      if (is_debug)
	ospf6_id2str (router_lsa->router_id, buf, sizeof (buf));
  ...
  if (abr_entry == NULL || abr_entry->path.area_id != oa->area_id ||
      CHECK_FLAG (abr_entry->flag, OSPF6_ROUTE_WAS_REMOVED) ||
...
ospf6_inter_area_prefix_lsa_show (struct vty *vty, struct ospf6_lsa *lsa)
{
  ...
  vty_out (vty, "     Prefix: %s/%d%s", buf,
           ospf6_af_prefixlen6 (ospf6, prefix_lsa->prefix.prefix_length), VNL);
