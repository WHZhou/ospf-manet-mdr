...
{
  struct ospf6_lsa *old, *self, *delayed, *new;
  ...
  delayed = ospf6_remove_matching_delayed_lsa (old);

  if (self && delayed)
    {
      /* a delayed LSA should be more recent so use it instead */
      if (ospf6_lsa_compare (self, delayed) >= 0)
        {
          self = delayed;
        }
      else
        {
          zlog_err ("%s: ERROR delayed LSA less recent than "
                    "flooded LSA: %s", __func__, delayed->name);
        }
    }
