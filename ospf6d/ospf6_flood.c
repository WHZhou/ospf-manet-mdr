...
{
  struct listnode *node;
  struct ospf6_lsa *delayed;

  for (ALL_LIST_ELEMENTS_RO (ospf6->delayed_lsa_list, node, delayed))
    if (OSPF6_LSA_IS_SAME (delayed, lsa))
      return node;
...
struct ospf6_lsa *
ospf6_remove_matching_delayed_lsa (struct ospf6_lsa *lsa)
{
  struct listnode *node;

  node = ospf6_find_matching_delayed_lsa_node (lsa);
  if (node)
    {
      struct ospf6_lsa *delayed = listgetdata (node);
      THREAD_OFF (delayed->delay);
      list_delete_node (ospf6->delayed_lsa_list, node);
      return delayed;
    }
  ...
  if (delayed_lsa_node)
    {
      ...
      if (lsa->header->seqnum == delayed_lsa->header->seqnum ||
          ospf6_lsa_compare (delayed_lsa, lsa) >= 0)
        {
	  /* delayed lsa is superseded by this lsa */
	  if (IS_OSPF6_DEBUG_ORIGINATE_TYPE (lsa->header->type))
	  ...
	  ospf6_lsa_delete (delayed_lsa);
        ...
        {
          zlog_err ("%s: ERROR: Suppress updating LSA "
                    "(more recent LSA already delayed): %s",
                    __func__, lsa->name);
	  ospf6_lsa_delete (lsa);
	  return;
        }
...
ospf6_lsa_purge (struct ospf6_lsa *lsa)
{
  ...
  /* remove any delayed self-originated LSA */
  delayed = ospf6_remove_matching_delayed_lsa (lsa);
  if (delayed)
    ospf6_lsa_delete (delayed);
