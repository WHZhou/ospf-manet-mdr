...
#define OSPF6_HELLO_INTERVAL_JITTER_MSEC 100
...
ospf6_header_print (struct ospf6_header *oh)
{
  ...
  ospf6_id2str (oh->router_id, router_id, sizeof (router_id));
  ospf6_id2str (oh->area_id, area_id, sizeof (area_id));
...
ospf6_hello_print (struct ospf6_header *oh)
{
  ...
  ospf6_id2str (hello->drouter, drouter, sizeof (drouter));
  ospf6_id2str (hello->bdrouter, bdrouter, sizeof (bdrouter));
  ...
  for (p = (char *) ((caddr_t) hello + sizeof (struct ospf6_hello));
       ...
       p += sizeof (u_int32_t))
    {
      ospf6_id2str (*(u_int32_t *) p, neighbor, sizeof (neighbor));
...
ospf6_lsreq_print (struct ospf6_header *oh)
{
  ...
  for (p = (char *) ((caddr_t) oh + sizeof (struct ospf6_header));
       ...
       p += sizeof (struct ospf6_lsreq_entry))
    {
      ...
      ospf6_id2str (e->adv_router, adv_router, sizeof (adv_router));
      ospf6_id2str (e->id, id, sizeof (id));
		  ...
		  struct ospf6_lls_header *lls)
{
   ...
   * compatibility).
   */
  if (OSPF6_OPT_ISSET (oi->area->options, OSPF6_OPT_AF, 1) &&
      !ospf6_af_is_ipv6_unicast (oi->area->ospf6))
    {
      if (!OSPF6_OPT_ISSET (hello->options, OSPF6_OPT_AF, 1))
        {
          if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
            zlog_debug ("Address Family Bit mismatch");
          return;
        }
    }
  ...
  err = ospf6_neighbor_hello_recv (on, oh, lls);
  if (err)
    {
  ...
  if (oi->type == OSPF6_IFTYPE_MDR)
    {
      ospf6_mdr_hello_recv (on, oh, lls);
      return;
    }
                          ...
                          struct ospf6_neighbor *on)
{
  ...
  for (p = (char *) ((caddr_t) dbdesc + sizeof (struct ospf6_dbdesc));
      ...
      if (mine != NULL && ospf6_lsa_compare (his, mine) <= 0)
	{
	  ...
	  mine_summary =
	    ospf6_lsdb_lookup (his->header->type, his->header->id,
  ...
  if (on->ospf6_if->type == OSPF6_IFTYPE_MDR)
    {
      if (OSPF6_OPT_ISSET (dbdesc->options, OSPF6_OPT_L, 1))
        {
	  assert (lls);
          ospf6_mdr_process_dd_tlv (on, lls);
        }
	  ...
	  ospf6_neighbor_exstart (on);
	      ...
	      //so cancel the pending DD packet send scheduled by
...
static unsigned
ospf6_packet_examin (struct ospf6_header *oh, unsigned bytesonwire)
{
  ...
  bytesonwire = ntohs (oh->length); /* ignore any trailing data */
  if
  (
...
ospf6_rxpacket_examin (struct ospf6_interface *oi, struct ospf6_header *oh, const unsigned bytesonwire)
{
  ...
  /* Instance-ID check */
  if (oh->instance_id != oi->area->ospf6->instance_id)
  {
    if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
      zlog_debug ("%s: Instance-ID mismatch (my %u, rcvd %u)", __func__, oi->area->ospf6->instance_id, oh->instance_id);
    return MSG_NG;
  }
                     ...
                     struct ospf6_interface *oi, struct ospf6_header *oh)
{
  ...
  else if (on->state != OSPF6_NEIGHBOR_EXCHANGE &&
	   on->state != OSPF6_NEIGHBOR_LOADING &&
	   on->state != OSPF6_NEIGHBOR_FULL)
    {
      if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
                  ...
                  struct ospf6_interface *oi, struct ospf6_header *oh)
{
  ...
  for (p = (char *) ((caddr_t) oh + sizeof (struct ospf6_header));
       ...
       p += sizeof (struct ospf6_lsa_header))
    {
      ...
      if (on->ospf6_if->type == OSPF6_IFTYPE_MDR)
	ospf6_mdr_neighbor_store_ack (on, his);
      ...
      // Delete backupwait neighbor corresponding to source of LSAck
      if (on->ospf6_if->type == OSPF6_IFTYPE_MDR &&
          mine->backupWaitTimer && ospf6_lsa_compare (his, mine) == 0)
        {
          ospf6_backupwait_lsa_neighbor_delete (mine, on);
        }
...
{
  int sockfd;
  unsigned int len, ospflen, extra;
  ...
  if (oi->state <= OSPF6_INTERFACE_LOOPBACK)
    {
      if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
        zlog_debug ("%s: Ignore message on non-active interface %s",
  ...
  ospflen = ntohs (oh->length);
  extra = len - ospflen;
  llsopt = ospf6_lls_option_isset (oh);
  lls = NULL;
  ...
  if (extra)
      ...
      if (llsopt)
	{
	  ...
	  lls = (struct ospf6_lls_header *) ((char *) oh + ospflen);
	  debug = IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV);
	  llserr = ospf6_lls_validate_datablock (lls, extra, debug);
	  if (llserr)
	    {
	      zlog_warn ("%s: removing invalid LLS data", __func__);
	      lls = NULL;
	      len = ospflen;
	      ospf6_lls_option_clear (oh);
	    }
	}
      else
	{
	  zlog_warn ("%s: ignoring trailing %u bytes of message data",
		     __func__, extra);
	  len = ospflen;
	}
    }
  else
    {
      if (llsopt)
	{
	  zlog_warn ("%s: L-bit was set but no LLS data block found: "
		     "clearing the L-bit", __func__);
	  ospf6_lls_option_clear (oh);
	}
    }
  ...
  if (IS_OSPF6_DEBUG_MESSAGE (oh->type, RECV))
    {
      ospf6_addr2str6 (&src, srcname, sizeof (srcname));
      ospf6_addr2str6 (&dst, dstname, sizeof (dstname));
  ...
  switch (oh->type)
    {
      case OSPF6_MESSAGE_TYPE_HELLO:
	ospf6_hello_recv (&src, &dst, oi, oh, lls);
        break;

      case OSPF6_MESSAGE_TYPE_DBDESC:
	ospf6_dbdesc_recv (&src, &dst, oi, oh, lls);
        break;
            ...
            struct ospf6_interface *oi, struct ospf6_header *oh, int length)
{
  ...
  oh->instance_id = oi->area->ospf6->instance_id;
  oh->reserved = 0;
  ...
  if (IS_OSPF6_DEBUG_MESSAGE (oh->type, SEND))
    {
      ...
      switch (oh->type)
		...
		if (OSPF6_OPT_ISSET (hello->options, OSPF6_OPT_L, 1) &&
		  ...
		  /* assume lls data block is valid */
		  lls = (struct ospf6_lls_header *) ((char *) oh +
						     ntohs (oh->length));
		else
		  lls = NULL;
...
ospf6_schedule_hello (struct ospf6_interface *oi)
{
  ...
  msec = 1000 * oi->hello_interval - OSPF6_HELLO_INTERVAL_JITTER_MSEC / 2 +
    ospf6_random (OSPF6_HELLO_INTERVAL_JITTER_MSEC);
  ...
  if (oi->state <= OSPF6_INTERFACE_LOOPBACK)
    {
      if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_HELLO, SEND))
        zlog_debug ("Unable to send Hello on non-active interface %s",
                   oi->interface->name);
      return 0;
    }
...
ospf6_dbdesc_send (struct thread *thread)
{
  ...
  else if (on->ospf6_if->type == OSPF6_IFTYPE_MDR &&
           on->ospf6_if->mdr.AdjConnectivity > OSPF6_ADJ_FULLYCONNECTED)
    {
      u_int size = sizeof (struct ospf6_lls_header) +
        sizeof (struct ospf6_tlv_header) + sizeof (struct ospf6_mdr_dd_tlv);
	  ...
	  hdrlen = p - sendbuf;
          ...
          lls = (struct ospf6_lls_header *)p;
	  p += sizeof (struct ospf6_lls_header);

          /* Router TLV */
          //mdr, mbdr, parent, bparent already up to date ??? XXX
          p += ospf6_mdr_append_dd_tlv (on->ospf6_if, p);
	  ospf6_set_lls_header (lls, size);
        }
    }
...
static int
ospf6_lsreq_lsa_cmp (void *a, void *b)
{
  struct ospf6_lsa *lsa1 = a;
  struct ospf6_lsa *lsa2 = b;

  return lsa1->retrans_count - lsa2->retrans_count;
}

static unsigned char *
ospf6_lsreq_add_entries (unsigned char *p, size_t packet_max,
			 struct list *lsalist, struct timeval *now)
{
  ...
  for (;;)
    {
      /* MTU check */
      if (p + sizeof (struct ospf6_lsreq_entry) > sendbuf + packet_max)
	break;

      head = listhead (lsalist);
      if (head == NULL)
	break;

      lsa = listgetdata (head);
      list_delete_node (lsalist, head);
...
ospf6_lsreq_send (struct thread *thread)
{
  ...
  struct ospf6_header *oh;
  long request_rxmt_delay;
  struct timeval now, min_originated;
  struct list *reqlist, *candidate_reqlist;
  struct ospf6_lsa *lsa;
  u_char *p;
  ...
  if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSREQ, SEND))
    zlog_debug ("LSReq total request list size: %u", on->request_list->count);

  maxentries = (ospf6_packet_max (oi) - sizeof (struct ospf6_header)) /
    sizeof (struct ospf6_lsreq_entry);
  if (maxentries > on->request_list->count)
    maxentries = on->request_list->count;

  if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSREQ, SEND))
    zlog_debug ("Maximum number of entries in LSReq packet: %u", maxentries);

  memset (sendbuf, 0, iobuflen);
  oh = (struct ospf6_header *) sendbuf;

  /* assume an outstanding request was lost after this long */
  request_rxmt_delay = 1000 * oi->rxmt_interval / 4;
  if (request_rxmt_delay < oi->flood_delay)
    request_rxmt_delay = oi->flood_delay;

  /* for LSAs to request now */
  reqlist = list_new ();

  /* for LSAs that were requested more than request_rxmt_delay ago */
  candidate_reqlist = list_new ();
  /* LSAs requested the fewest number of times are sent first */
  candidate_reqlist->cmp = ospf6_lsreq_lsa_cmp;
  ...
  for (lsa = ospf6_lsdb_head (on->request_list); lsa;
       lsa = ospf6_lsdb_next (lsa))
    {
      if (!timerisset (&lsa->originated))
	{
	  listnode_add (reqlist, lsa);
	  if (listcount (reqlist) == maxentries)
	    {
	      ospf6_lsa_unlock (lsa);
	      break;
	    }
	}
      else if (timersub_msec (&now, &lsa->originated) >= request_rxmt_delay)
	{
	  listnode_add_sort (candidate_reqlist, lsa);

	  /* we can only send a total of maxentries requets */
	  while (listcount (reqlist) +
		 listcount (candidate_reqlist) > maxentries)
	    {
	      list_delete_node (candidate_reqlist,
				listtail (candidate_reqlist));
	      if (listcount (candidate_reqlist) == 0)
		break;
	    }
	}
      else
	{
	  /* note min_originated is only used if both reqlist and
	     candidate_reqlist are empty */
	  if (timercmp (&lsa->originated, &min_originated, <))
	    min_originated = lsa->originated;
	}
    }

  if (listcount (reqlist) == 0 && listcount (candidate_reqlist) == 0)
    {
      long delay;

      delay = request_rxmt_delay - timersub_msec (&now, &min_originated);
      assert (delay > 0);
      assert (delay <= request_rxmt_delay);

      if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSREQ, SEND))
	{
	  zlog_debug ("All LSAs have already been requested");
	  zlog_debug ("Scheduling next LSReq in %ld msec", delay);
	}

      on->request_retrans_wait = true;
      on->thread_send_lsreq =
	thread_add_timer_msec (master, ospf6_lsreq_send, on, delay);

      list_delete (reqlist);
      list_delete (candidate_reqlist);
  ...
  p = ospf6_lsreq_add_entries (p, ospf6_packet_max (oi), reqlist, &now);
  assert (listcount (reqlist) == 0);

  p = ospf6_lsreq_add_entries (p, ospf6_packet_max (oi),
			       candidate_reqlist, &now);
  assert (listcount (candidate_reqlist) == 0 ||
	  p + sizeof (struct ospf6_lsreq_entry) >
	  sendbuf + ospf6_packet_max (oi));

  oh->type = OSPF6_MESSAGE_TYPE_LSREQ;
  oh->length = htons (p - sendbuf);

  ospf6_send (oi->linklocal_addr, &on->linklocal_addr,
	      oi, oh, ntohs (oh->length));

  if (IS_OSPF6_DEBUG_MESSAGE (OSPF6_MESSAGE_TYPE_LSREQ, SEND))
    zlog_debug ("Scheduling next LSReq in %u msec",
		1000 * oi->rxmt_interval);

  /* set next thread */
  on->request_retrans_wait = false;
  on->thread_send_lsreq =
    thread_add_timer (master, ospf6_lsreq_send, on, oi->rxmt_interval);
...
ospf6_lsupdate_send_neighbor (struct thread *thread)
{
  ...
  /* lsupdate_list: lists LSAs which don't need to be
  ...
  for (lsa = ospf6_lsdb_head (on->lsupdate_list); lsa;
       lsa = ospf6_lsdb_next (lsa))
    {
      ...
      if ( (p - sendbuf + (unsigned int)OSPF6_LSA_SIZE (lsa->header))
          > ospf6_packet_max(on->ospf6_if))
        {
          // RGO. If a single LSA causes LSU to exceed MTU, it is
          // necessary to send the LSA anyway.
          if (num > 0)
            {
	      ospf6_lsa_unlock (lsa);
	      break;
  ...
  rxmt_msec = 1000 * on->ospf6_if->rxmt_interval;
  for (lsa = ospf6_lsdb_head (on->retrans_list); lsa;
       lsa = ospf6_lsdb_next (lsa))
    {
      lsa_rxmt_msec = 1000 * on->ospf6_if->rxmt_interval -
	elapsed_msec (&lsa->rxmt_time);
      if (lsa_rxmt_msec < 1) //thread_add_timer only works on 1 msec increment
        {
          ospf6_lsa_unlock (lsa);
          rxmt_now = true;
          break;
  ...
  if (rxmt_now)
	   ...
	   lsa = ospf6_lsdb_next (lsa))
	{
	  lsa_rxmt_msec = 1000 * on->ospf6_if->rxmt_interval -
	    elapsed_msec (&lsa->rxmt_time);
          if (lsa_rxmt_msec > on->ospf6_if->flood_delay)
            {                   //flood lsa that have expired or will expire within flood_delay msec
              //this lsa should not be retransmitted yet
              if (lsa_rxmt_msec < rxmt_msec)
                rxmt_msec = lsa_rxmt_msec;
              continue;
            }
	  //set the time lsa is retransmitted
          ...
          for (i = 0; i < num - rxmt; i++)
            {
              if (lsa->header->type == ((struct ospf6_lsa_header *) u)->type
                  ...
                  && lsa->header->seqnum ==
                  ((struct ospf6_lsa_header *) u)->seqnum
                  && lsa->header->length ==
                  ((struct ospf6_lsa_header *) u)->length)
                {
                  same = true;
                  break;
                }
...
struct thread *
ospf6_send_lsupdate_delayed_msec (struct thread_master *m,
                                  int (*func) (struct thread *), void *arg,
                                  long timer_msec, struct thread *t)
{
  long msec_remaining;
  if (!t)
    {
  ...
  msec_remaining = -elapsed_msec (&t->u.sands);
  if (msec_remaining > timer_msec)
    {
      THREAD_OFF (t);
      t = thread_add_timer_msec (m, func, arg, timer_msec);
    }
...
ospf6_lsack_send_interface (struct thread *thread)
{
  ...
  for (lsa = ospf6_lsdb_head (oi->lsack_list); lsa;
       lsa = ospf6_lsdb_next (lsa))
    {
      ...
      if (oi->type == OSPF6_IFTYPE_MDR)
        {
	  long lsack_delay_msec;

          // Do not send lsack until rxmt_interval - ackInterval - 0.5 sec
          // after LSA was rcvd.  This should be 7 - 1 - 0.5 = 5.5 sec.
	  lsack_delay_msec =
	    1000 * oi->rxmt_interval - oi->mdr.ackInterval - 500;
          if (lsack_delay_msec > 0 &&
	      elapsed_msec (&lsa->rxmt_time) < lsack_delay_msec)
            continue;           // Not yet time to send lsack
        }
  ...
  else if ((unsigned int) (p - sendbuf) > sizeof (struct ospf6_header))
    ospf6_send (oi->linklocal_addr, &allspfrouters6, oi, oh,
                ntohs (oh->length));
